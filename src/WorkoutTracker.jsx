...
const WorkoutTracker = () => {
  ...
  useEffect(() => {
    const loadGoogleAPI = () => {
      ...
      script.onerror = () => {
        ...
        alert('Failed to load Google Drive API. Please check your internet connection and refresh the page.');
      };
    ...
    const initializeGapi = async () => {
      try {
        ...
        // Initialize with just client ID and scope - no API key needed for OAuth
        ...
        if (authInstance && authInstance.isSignedIn.get()) {
          ...
          console.log('ℹ️ User not signed in yet');
        }
      } catch (error) {
        ...
        let errorMsg = 'Error initializing Google Drive.\n\n';
        
        if (error.error === 'idpiframe_initialization_failed') {
          errorMsg += 'Issue: Cookies may be blocked.\n\n';
          errorMsg += 'Fix: Enable third-party cookies for this site, or try in a different browser.';
        } else if (error.details) {
          errorMsg += 'Details: ' + error.details;
        } else if (error.message) {
          errorMsg += 'Details: ' + error.message;
        } else {
          errorMsg += 'Unknown error. Check the console (F12) for details.\n\n';
          errorMsg += 'Make sure:\n';
          errorMsg += '1. Your deployment URL is added to Google Cloud Console\n';
          errorMsg += '2. You\'re using HTTPS (not HTTP)\n';
          errorMsg += '3. Third-party cookies are enabled';
        }
  ...
  const connectGoogleDrive = async () => {
    if (!gapiLoaded) {
      ...
      while (retries < maxRetries && !gapiLoaded) {
        await new Promise(resolve => setTimeout(resolve, 500));
        retries++;
      }
      
      if (!gapiLoaded) {
        alert('Google API is still loading. Please wait a few seconds and try again.\n\nIf this persists, try refreshing the page.');
        return;
      }
    }
    ...
    try {
      ...
      await loadFromGoogleDrive(token);
      ...
      if (error.error === 'popup_closed_by_user') {
        alert('Sign-in cancelled. Click "Sync Drive" to try again.');
      } else {
